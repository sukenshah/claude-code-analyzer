...
import type { SessionDetail, RawMessage, TurnRecord, ToolUse } from "@/lib/types";
...
export function SessionPage() {
  ...
  const orderedMessages = msgOrder === "asc" ? filteredMessages : [...filteredMessages].reverse();
  const { page: msgPage, setPage: setMsgPage, paged: pagedMessages, total: msgTotal } = usePagination(orderedMessages);
      ...
      <div className="stat-grid">
        <div className="stat-card">
          ...
          <div className="stat-value small">
            <Link href={`/project/${encodeURIComponent(session.projectKey)}`} className="link">
              {session.projectName}
            </Link>
          </div>
        </div>
      ...
      <section className="card">
        <div className="section-header">
          ...
          <div className="sort-buttons">
            ...
            <span style={{ marginLeft: 8 }}>Order:</span>
            <button className={`btn-sort ${msgOrder === "asc" ? "active" : ""}`} onClick={() => { setMsgOrder("asc"); setMsgPage(1); }}>Oldest first</button>
            <button className={`btn-sort ${msgOrder === "desc" ? "active" : ""}`} onClick={() => { setMsgOrder("desc"); setMsgPage(1); }}>Newest first</button>
          </div>
        </div>
        <input
        ...
        {m.toolUses.length > 0 && (
          <div className="msg-tools">
            {m.toolUses.map((tool: ToolUse, i) => (
              <Fragment key={i}>
                <span className="badge msg-tool-badge">
